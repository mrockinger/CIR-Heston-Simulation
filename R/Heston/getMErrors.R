...
getHestonMErrors = function(S0, v0, mu, kappa, theta, sigma, rho, T, n, M, NRuns, names, nE, nI, nO, setup, approach){
  ...
  if(setup == 'C'){
    ...
    momth = MomentsBates(mu = mu, kappa = kappa, theta = theta, sigma = sigma, rho = rho, lambda = 0, muj = 0, vj = 0, t = T, v0 = v0, conditional = TRUE)[c(1, 5:7)]
    ...
    for(k in 1:NRuns){
      ...
      for(j in 1:length(n)){
        ...
        Errors[[j]] = foreach(i = 1:length(names), .combine = 'cbind', .packages = c("Bessel", "actuar", "nleqslv", "sfsmisc")) %dopar% TreatReturns(Model = names[i], S0 = S0, v0 = v0, kappa = kappa, theta = theta, sigma = sigma, mu = mu, rho = rho, T = T, n = n[j], M = M, momth = momth, setup = setup)
    ...
    if(approach == 1){ # fix n = 1 and draw v0 from the steady state distribution
      ...
      for(k in 1:NRuns){
        ...
        for(j in 1:length(T)){
          ...
          ErT = MomentsBates(mu = mu, kappa = kappa, theta = theta, sigma = sigma, rho = rho, lambda = 0, muj = 0, vj = 0, t = T[j], v0 = 1, conditional = FALSE)[c(1, 5:7)]
          varT = ErT[2] - ErT[1]^2
          
          momth = cbind(ErT[1], varT, ErT[5:6])
        
          Errors[[j]] = foreach(i = 1:length(names), .packages = c("Bessel", "actuar", "nleqslv", "sfsmisc")) %dopar% TreatReturns(S0 = S0, v0 = v0, kappa = kappa, theta = theta, sigma = sigma, mu = mu, rho = rho, T = T[j] , n = n, M = M, momth = momth)
      ...
      for(k in 1:NRuns){
        ...
        for(j in 1:length(T)){
          ...
          Errors[[j]] = foreach(i = 1:length(names), .combine = 'cbind', .packages = c("Bessel", "actuar", "nleqslv", "sfsmisc")) %dopar% TreatVarianceUC(Model = names[i], v0 = v0, kappa = kappa, theta = theta, sigma = sigma, T = T[j], n = n, M = M, momth = momth)
