...
for t=1:n
    ...
    % Here, there seems to be a flaw in the code, since for LNC we will
    % most certainly compute v like this and especially here, we have huge
    % differences in errors comparing with R results
    I = find(psi > psic);

        % test =find(U(t,I) < (psi(I) - 1) ./ (psi(I) + 1));
